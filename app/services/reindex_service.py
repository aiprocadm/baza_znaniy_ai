...
class ReindexService:
    ...
    def reindex_document(self, *, document_id: str, idempotency_key: str | None = None, dry_run: bool = False) -> ReindexResult:
        ...
        switched = False
        try:
            logger.info("reindex stage=copy document_id=%s idempotency_key=%s", document_id, idempotency_key)
            ...
            logger.info("reindex stage=verify document_id=%s copied=%s idempotency_key=%s", document_id, copied, idempotency_key)
            self.store.validate_collection_not_empty(temp_collection)
            if dry_run:
                logger.info("reindex stage=rollback document_id=%s reason=dry_run idempotency_key=%s", document_id, idempotency_key)
            ...
            logger.info("reindex stage=switch document_id=%s idempotency_key=%s", document_id, idempotency_key)
            ...
            logger.exception("reindex stage=rollback document_id=%s idempotency_key=%s switched=%s", document_id, idempotency_key, switched)
            if not switched:
                self.store.delete_collection_safe(temp_collection)
            raise
