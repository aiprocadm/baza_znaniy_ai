...
- **Service wrapper with in-memory fallback:** `app/services/vectorstore.py`. The fallback is a substring scan, **not a real vector search** — if Qdrant/FAISS fails at runtime, search silently degrades to grep. An active multi-sprint refactor unifies the filter contract across backends through `SearchFilters` — when touching search, check `docs/superpowers/plans/*vectorstore*` for current scope and the still-`skip`ped tests it tracks.
