...
- **C — Russian reranker.** `DEFAULT_MODEL_NAME` in `app/services/kb_rerank.py`
  ...
  - **RESULT (2026-06-06 — in-process bge-m3 + bge-reranker-v2-m3, curated golden
    ...
    **LATENCY: FAIL on CPU** — warm **~52 s/query** (median 51, p95 65, max 83 s),
    ~237 s cold start on a Core Ultra 9 185H (560M XLM-R cross-encoder × 20 long
    chunks). Decision: `DEFAULT_MODEL_NAME` swapped to `bge-reranker-v2-m3` (both
    files + `.env.example`) as the correct RU model, but `KB_RERANK_ENABLED` stays
