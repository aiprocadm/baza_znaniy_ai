...
> **ПОПРАВКА (по итогам самого свипа, 2026-07-02):** цифра «p95 ≈ 1.2 с» выше —
> ошибка атрибуции: 1228 мс были замерены для *студента* rubert-tiny2
> (`scripts/quantize_reranker.py`, max_length=256), а не для teacher'а. Реальный
> teacher на прод-пути стоит ~0.9 с/кандидата (p95 ≈ 21 с при k=20) — см. секцию
> «Candidate/latency sweep (Track B)» в runbook 2026-06-15.
